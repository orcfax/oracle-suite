...
variables {
  musig_pairs = explode(env("CFG_ITEM_SEPARATOR", "\n"), env("CFG_SYMBOLS", env("CFG_MUSIG_PAIRS", "")))
}
...
musig {
  ...
  dynamic "tick" {
    for_each = [
      for v in var.models : v
      # If CFG_SYMBOLS (or CFG_MUSIG_PAIRS) is set to a list of assets, filter by that list
