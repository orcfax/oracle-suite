...
func TestGenericHTTP_FetchDataPointsOrcfax(t *testing.T) {
	...
	for _, tt := range testCases {
		t.Run(tt.name, func(t *testing.T) {
			...
			if tt.expectError {
				// If we get this far we don't want to process the data
			...
			for _, dataPoint := range points {
				...
				// Using expected strings we can get some rudimentary testing
				// without knowing the whole data structure.d
				for _, value := range tt.expectedStrings {
					if !strings.Contains(fmt.Sprint(httpResponse.Body), value) {
						t.Errorf("value '%s' not in '%s'", value, httpResponse.Body)
					}
				}
			}
		})
