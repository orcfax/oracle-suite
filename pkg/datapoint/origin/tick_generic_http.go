...
func (g *TickGenericHTTP) FetchDataPoints(ctx context.Context, query []any) (map[any]datapoint.Point, error) {
	...
	for url, pairs := range g.group(pairs) {
		...
		// The shape of the response may be a dictionary.
		...
		collectorMessage := make(map[string]any)
		...
		if err != nil {
			collectorJSON = []byte(fmt.Sprintf("{\"no data\": \"%s\"}", err))
		}
		...
		for pair, point := range resPoints {
			...
			// Add Orcfax metadata.
			point.Meta["response"] = b64.StdEncoding.EncodeToString([]byte(collectorJSON))
